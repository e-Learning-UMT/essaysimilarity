...
class qtype_essaysimilarity_renderer extends qtype_renderer {
    ...
    public function formulation_and_controls(question_attempt $qa, question_display_options $options) {
        ...
        $answerattr = ['class' => 'answer'];
        if ($options->readonly) {
            $answerattr['style'] = 'background-color: #fff;
                              padding: 0.1em;
                              margin: 10px 0px;';
        }

        $result .= html_writer::tag('div', $answer, $answerattr);
    ...
    protected function specific_feedback(question_attempt $qa) {
        ...
        $showteacher = empty($this->displayoptions->context) ? false :
            has_capability('mod/quiz:grade', $this->displayoptions->context);
        $showstudent = $showteacher ? false :
            has_capability('mod/quiz:attempt', $this->displayoptions->context);
        ...
        if ($showanswerkey) {
            $output .= html_writer::tag('h5', get_string('answerkey', $pluginname));
        ...
        if ($showtextstats) {
            ...
            foreach ($statsitem as $item) {
                $label = get_string($item, $pluginname);

                if ($strmanager->string_exists($item . '_help', $pluginname)) {
                    $label .= $this->help_icon($item, $pluginname);
                }
            ...
            $output .= html_writer::tag('h5', get_string('textstatistics', $pluginname));
            $output .= html_writer::table($table);
        }

        // Display plagiarism links if any.
        $response = $qa->get_last_qt_data();
        $plagiarismcontent = $question->get_plagiarism($response);
        if (!empty($plagiarismcontent)) {
            $output .= html_writer::tag('h5', get_string('plagiarismcheck', $pluginname));

            $plagiarism = [];
            foreach ($plagiarismcontent as $link) {
        ...
        $floatprecision = isset($this->displayoptions->markdp) ? $this->displayoptions->markdp : 0;
        if ($showfeedback) {
            $output .= html_writer::tag('h5', get_string('feedbacksection', $pluginname));

            $maxgrade = $qa->get_max_mark();

            $step = $qa->get_last_step_with_behaviour_var('finish');
            if ($step->get_id()) {
                $grade = format_float($step->get_fraction() * $maxgrade, $floatprecision);
            } else {
                $grade = $qa->format_mark($floatprecision);
            }

            $output .= html_writer::tag('p', get_string('feedback', $pluginname, $grade));
        }
        ...
        if ($step->get_id()) {
            ...
            'manualgrade' => format_float($step->get_behaviour_var('mark'), $floatprecision),
            ];
     ...
     *
     * @param question_attempt $qa the question attempt to display.
     * @param question_display_options $options controls what should and should not be displayed. Used to get the context.
    ...
    public function files_input(question_attempt $qa, question_display_options $options) {
        ...
        [$context, $course, $cm] = get_context_info_array($options->context->id);

        $maxbytes = $context ? $course->maxbytes : $this->page->course->maxbytes;
